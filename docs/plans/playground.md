...
static web page at `https://panset.github.io/cinegram/playground/` where a developer
...
- **Hosting** *(revised 2026-08-14 — the Pages demo site landed first)*:
  GitHub Pages is already live at `https://panset.github.io/cinegram/`
  serving the committed `docs/` directory (`build_type: legacy`, branch
  `main`, path `/docs`; regenerated by `bazel run //site:sync`, kept fresh by
  `//site:site_test`). The playground must NOT displace it. A new Actions
  workflow deploys `_site = docs/** + assembled playground at /playground/`,
  preserving every existing demo URL; the Pages source is then flipped to
  "GitHub Actions" (`gh api -X PUT repos/panset/cinegram/pages -f
  build_type=workflow` — the executor CAN do this, gh is authenticated; do it
  only AFTER the workflow has merged and produced a green run, or the site
  404s in the gap). `docs/` stays committed and tested exactly as today — the
  workflow uploads it verbatim, never rebuilds it. The ~15–20 MB `.wasm` is
  therefore never committed. RELEASING.md §"Adding a distribution channel"
  sanctions this: the playground serves *content*, not the CLI binary, so it
  rides `main` rather than adding a `release.yml` channel job.
...
- `Cinegram.mount(root, timeline, opts)` (runtime.js) accepts
  ...
  listener when no explicit theme, the reel-era **window `resize`** listener
  (its comment already says "joins Player.dispose when that exists"), plus
  `cancelAnimationFrame`).
- Reel mode (added after this plan was first written; `?reel`, auto-follow
  camera, segmented bar) needs nothing from the playground: `isReel()`
  returns false whenever `opts.hash === false`, which the playground always
  passes, so a stray `?reel` in a playground URL cannot flip an embedded
  mount into reel mode.
- `stateDiagram-v2` is now a third diagram type; `examples/raft-election.dgm`
  and `examples/tcp-connection.dgm` exercise it and belong in the examples
  picker.
- RELEASING.md §"Adding a distribution channel" explicitly covers a
  playground: a channel serving content (not the CLI binary) rides `main`
  like the demo site instead of adding a `release.yml` job.
...
(carry over the "wire format" doc comments at main.go:~348-354, ~480-486;
line numbers rechecked 2026-08-14):
- `Envelope` (was `jsonEnvelope`, main.go:~355-358): `Timeline *ir.Timeline`
  + `Diagnostics []Diagnostic`, same JSON tags.
- `Diagnostic` (was `jsonDiagnostic`, main.go:~482-497): file/line/col/
  severity/message/hint(omitempty), identical tags.
- `Collect(bags []*diag.Bag) ([]Diagnostic, int)` (was `collectDiagnostics`,
  main.go:~499) — verbatim body; always returns a non-nil slice.

**Modify `cmd/cinegram/main.go`**: delete the moved types/func;
`writeEnvelope` (~360) STAYS (it does output I/O) but takes/builds
`envelope.Envelope`; update `cmdCompile` (envelope use ~289-301) and
`lintJSON` (~525) to use `envelope.Collect` / `envelope.Diagnostic`.

**Modify `cmd/cinegram/main_test.go`**: `TestEnvelopeAlwaysCarriesBothHalves`
(~194-236) keeps pinning `writeEnvelope` output; `mustCollect` (~238),
`TestLintJSONExitCodes` (~247-) switch to the envelope types.
...
style — no arrow functions, no modules; the file is now ~3155 lines after the
reel work — line numbers rechecked 2026-08-14):
- Constructor `function Player(` (~370): add `this._unbind = [];`
- Module-scope helper `own(player, target, type, fn, opts)` — addEventListener
  + push a remover onto `player._unbind`.
- Rewrite the SIX leaking sites to use it: window `resize` (~580 — its
  comment "joins Player.dispose when that exists" was written for this
  phase; delete that sentence when it comes true), matchMedia `change`
  (~647-648, keep the `addListener` legacy fallback, pushing a manual
  remover), `document` keydown (~663, only when `keys !== 'scoped'`),
  `window` hashchange (~674, only when `hash !== false`), `window`
  pointermove (~1009) and pointerup (~1024) in `bindStageGestures`.
- New `Player.prototype.dispose()` near `pause`: `this.pause()`, run
...
Verified multi-file sets (rechecked 2026-08-14): `k8s-request.dgm` and
`websocket-handshake.dgm` each also need `pod-a.dgm`; `oidc-login.dgm` needs
the five `frames/*.svg`. All other examples are single-file (`embedded.md`,
`*.narrate.md`, `payment-checkout.fail.png`, `frames/README.md` are not
examples). Include `raft-election.dgm` and `tcp-connection.dgm` so the picker
covers all three diagram types (flowchart, sequenceDiagram, stateDiagram-v2).
...
*(Reworked 2026-08-14: Pages is already live serving committed `docs/` —
`build_type: legacy`, branch `main`, path `/docs` — as the demo site. The
playground joins it at `/playground/` without displacing anything.)*
...
steps: checkout → setup-bazelisk → `bazel test //...` (qualify; this also
runs `//site:site_test`, so a stale committed `docs/` blocks the deploy
exactly as it blocks CI today) →
`cp -R docs "$PWD/_site"` (the committed demo site, verbatim — the workflow
never rebuilds it; `//site:sync` remains the only writer) →
`bazel run //web/playground:site -- -o "$PWD/_site/playground"` →
...
`https://panset.github.io/cinegram/playground/` plus the dev-serve command;
the demos index template in `site/site.go` gains a "Try the playground" link
(then `bazel run //site:sync` to regenerate `docs/`); CLAUDE.md gains a short
subsection (playground assembled-not-committed, wasm BUILD is
hand-written/gazelle:ignore, envelope wire format now in `pkg/envelope`);
RELEASING.md's "rides `main` the way the GitHub Pages demo site does"
paragraph is updated to say the `pages` workflow now uploads `docs/` verbatim
plus the assembled playground (still no channel job — nothing here ships the
CLI binary).
...
**Post-merge cutover (sequence matters — flipping early 404s the live
site)**: (1) merge; (2) watch the `pages` workflow run to green — under
`build_type: legacy` the run's deploy step may no-op or the legacy builder
may race it, that is fine, what matters is the artifact builds; (3) flip the
source: `gh api -X PUT repos/panset/cinegram/pages -f build_type=workflow`
(gh is authenticated; no repo-settings click needed — but if the API call is
refused, the manual fallback is Settings → Pages → Source = "GitHub
Actions"); (4) re-run the workflow (`gh workflow run pages.yml`) and verify
`https://panset.github.io/cinegram/` still serves the demo index AND
`…/cinegram/playground/` boots (network tab: if Pages' content-type breaks
`instantiateStreaming`, the `arrayBuffer()` fallback covers it).
...
5. Textarea vs player keys: `keys:'scoped'` + runtime.js's input-tag guard
   (onKey, ~734) means typing never fights the transport.
6. Pages cutover: between flipping `build_type` to `workflow` and the first
   workflow deploy, the site serves whatever was last deployed. Mitigated by
   the Phase 5 sequence (merge → green run → flip → re-run → verify); the
   demo site stays on the legacy builder until the flip, so the window is
   minutes and reversible (`-f build_type=legacy` restores it).
