...
`main` the way the GitHub Pages demo site does: `docs/` is committed,
regenerated by `bazel run //site:sync`, and `//site:site_test` fails while it
is stale — so CI and the release's qualify step gate it without a channel job.
That pattern works only because nothing in it ships the binary; anything that
does must go through a channel job here.
