...
If the channel serves *content* rather than the binary (e.g. a GitHub Pages
demo site built from `examples/`), it still belongs here as a channel job, so
the site can never show a version of the pages that `bazel test` did not pass.
