...
// TestTheAxisFitButtonsSitInTheRowAndYieldToTheCamera pins where the two
// axis-fit controls live and the three things fitAxis has to get right.
//
// The row and not the rail: 09842fc thinned the rail to three on the argument
// that it is the narrowest and most contested space on the page, and Fill came
// back out of it for being a fourth. These are visible at rest without spending
// that space.
func TestTheAxisFitButtonsSitInTheRowAndYieldToTheCamera(t *testing.T) {
	js, css := codeOnly(string(runtimeJS)), string(runtimeCSS)
	...
	for _, want := range []string{
		...
		if !strings.Contains(js, want) {
			t.Errorf("runtime.js does not put an axis-fit button in the scenario row (%q). The "+
				"row is where they are reachable at rest — the minimap, which is the other "+
				"obvious home, is `is-off` exactly while the diagram still fits", want)
		}
	}
	for _, gone := range []string{"rail.appendChild(this.fitWideBtn)", "rail.appendChild(this.fitTallBtn)"} {
		if strings.Contains(js, gone) {
			t.Errorf("an axis-fit button is being built into the rail (%q); the rail is three "+
				"controls by argument, not by accident", gone)
		}
	}

	const sig = "Player.prototype.fitAxis = function (axis) {"
	start := strings.Index(js, sig)
	if start < 0 {
		t.Fatal("runtime.js no longer defines fitAxis, which is what both buttons call")
	}
	rest := js[start+len(sig):]
	if end := strings.Index(rest, "\n  Player.prototype."); end >= 0 {
		rest = rest[:end]
	}

	// Cine writes the same transform. Two things steering it is a fight decided
	// by whichever ran last, and the camera re-framing on the next beat would
	// make the press look broken.
	if !strings.Contains(rest, "this.setFollow(false)") {
		t.Error("fitAxis does not turn Cine off, so the camera will re-frame on the next step " +
			"and take the fitted axis back")
	}
	// Measured at scale 1: reading a scaled box and scaling it again compounds,
	// so a second press would walk the zoom away from the truth.
	if !strings.Contains(rest, "this.zoom = 1;") {
		t.Error("fitAxis does not reset the zoom before measuring, so it measures a box that " +
			"its own previous press already scaled")
	}
	// The same bounds the wheel obeys, so neither button can reach a scale a
	// reader could not have reached — or undone — by hand.
	if !strings.Contains(rest, "ZOOM_MIN") || !strings.Contains(rest, "ZOOM_MAX") {
		t.Error("fitAxis does not clamp to ZOOM_MIN/ZOOM_MAX; a large diagram in a small stage " +
			"can be driven to a scale the wheel cannot get back from")
	}

	// Icons in a row of words: the row's vertical sizing has to survive, so the
	// horizontal padding is set on its own. `padding` wholesale would take the
	// row's 3px top and bottom with it and stand these two taller than Present.
	fitBtn := ruleBody(t, css, ".dgm-fit-axis {")
	for _, want := range []string{"padding-left", "padding-right"} {
		if !strings.Contains(fitBtn, want) {
			t.Errorf("the axis-fit buttons do not set %s, so a 16px glyph carries the row's "+
				"12px word padding and the pair reads as two more words beside Present", want)
		}
	}
	if strings.Contains(fitBtn, "padding:") {
		t.Error("the axis-fit buttons set padding wholesale, which drops the scenario row's " +
			"vertical sizing and stands them taller than the picker and Present beside them")
	}
}
